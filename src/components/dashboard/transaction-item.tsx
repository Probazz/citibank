...
const typeConfig: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  ADMIN_CREDIT: { label: 'Admin Credit',  icon: <Building className="w-4 h-4" />,       color: 'bg-citi-green-light text-citi-green' },
  ADMIN_DEBIT:  { label: 'Admin Debit',   icon: <Building className="w-4 h-4" />,       color: 'bg-citi-red-light text-citi-red' },
};
  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-citi-gray-100 last:border-0 hover:bg-citi-gray-50 px-2 -mx-2 rounded-lg transition-colors cursor-pointer">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-citi-gray-800 truncate">{t.description}</p>
